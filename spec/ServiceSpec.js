...
describe('Service Tests', () => {
    ...
    it('Should return correct results when a valid JSON sent', async () => {
        ...
        const postRequestResult = await fetch(serviceUrl, {
            method: 'POST',
            headers: {
        ...
        const responseJson = await postRequestResult.json();

        assert.equal(JSON.stringify(responseJson), expectedResponse, "Response should have been a filtered list of shows");
        assert.equal(postRequestResult.status, "200", "Status should be 200 - OK");
    });

    it('Should return correct bad request error when JSON data sent does not contain a payload', async () => {
        let postData = JSON.stringify({
            "load": [],
            "skip": 0,
            "take": 2,
        ...
        assert.equal(JSON.stringify(responseJson), badRequestResponseJson, "Response message should have an error messsage");
        assert.equal(postRequestResult.status, "400", "Status should be 400 - Bad Request");
    });

    it('Should return an empty array if the sent JSON is valid but the list of shows do not meet the criteria ', async () => {
        let postData = JSON.stringify({
            "payload": [
                {
                    "country": "USA",
        ...
        assert.equal(JSON.stringify(responseJson), expectedResponse, "Response should have been an empty array");
        assert.equal(postRequestResult.status, "200", "Status should be 200 - OK");
    });
    ...
    it('Should return the exact results of the given example', async () => {
        let postData = JSON.stringify({
            "payload": [
                {
                    "country": "UK",
